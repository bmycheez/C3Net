...
parser.add_argument("--start_epochs", type=int, default=27, help="Number of training epochs")
parser.add_argument("--start_iters", type=int, default=5998, help="Number of training epochs")
parser.add_argument("--resume", type=str, default="net_38.0006.pth", help="Number of training epochs")
...
parser.add_argument("--outf", type=str, default="./checkpoint", help='path of log files')
...
def main():
    ...
    for epoch in range(opt.start_epochs, opt.epochs):
        ...
        for i, (imgn_train, img_train) in enumerate(loader_train, 0):
            if i < opt.start_iters and epoch == opt.start_epochs:
                continue
            # training step
            model.train()
            model.zero_grad()
            ...
            loss = color_loss(out_train, img_train) + criterion(out_train, img_train)
            # + 1e-4 * ((1 - s(out_train, img_train)) / 2.)
            loss /= 2
            loss.backward()
            optimizer.step()
            # '''
